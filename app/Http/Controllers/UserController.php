...
class UserController extends Controller
{
    ...
    public function liked(Request $request){
        ...
        foreach($user->likes as $like){
            ...
            array_push($displayPost, ["idPost" => $post->id,"like" => $cLike,"id" => $user->id, "user" => $userabo,"text" => $post->text, "path" => $path, "postedAt" => $post->posterA->toDateTimeString(), "imgP" => $user->medium->path]);
        }
        usort($displayPost, function ($item1, $item2) {
            return $item2['postedAt'] <=> $item1['postedAt'];
        });
